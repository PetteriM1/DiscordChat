...
public class PlayerListener implements Listener {
    ...
    public void onQuit(PlayerQuitEvent e) {
        if (Main.config.getBoolean("quitMessages") && e.getPlayer().spawned) {
            API.sendMessage(Main.config.getString("info_player_left").replace("%quit_message%", TextFormat.clean(textFromContainer(e.getQuitMessage()).replace("%player%", e.getPlayer().getName()))));
        }
    }
